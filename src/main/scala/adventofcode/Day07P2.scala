...
object Device:
    ...
    input.toList.foldLeft(Map[String, List[(String, Int)]]()) { (tree, line) =>
      line match
        case s"$$ cd $dir" =>
          ...
          if !tree.contains(pwd) then tree + (pwd -> List())
          else tree
        case s"dir $dir" =>
          val d = s"$pwd$dir/"
          tree + (d -> List())
  ...
  def sizes(tree: Map[String, List[(String, Int)]]): Map[String, Int] =
    tree.keys.toList.sorted.reverseIterator.foldLeft(Map[String, Int]()) { (map, key) =>
      ...
      val children = map
        .filter((ck, cv) => ck.startsWith(key) && ck.count(_ == '/') == key.count(_ == '/') + 1)
...
@main def Day07(args: String*): Unit =
  ...
  val max = sizes("/")
  val target = 30_000_000 - (70_000_000 - max)
  val result = sizes.values.filter(_ >= target).min
